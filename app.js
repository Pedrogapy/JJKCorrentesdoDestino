...
function renderSubcase(){
  ...
  <section style="margin-top:18px"><article class="panel"><h3>Anotações extras</h3><p class="note">${s.kiyomi}</p></article></section>`;
}
function renderProfessores(){
  layout(); const app=$('#app'); app.innerHTML=`<div class="top"><div><p class="eyebrow">Arquivo institucional</p><h2>Professores e direção</h2><p class="lead">Páginas individuais com informações que Kiyomi conseguiria manter no arquivo: dados públicos, padrões observados e anotações úteis de investigação.</p></div><div class="toolbar"><input id="search" class="search" placeholder="Buscar professor..." /></div></div><section class="grid three" id="profList"></section>`;
  const wrap=$('#profList'); DB.professors.forEach(p=>wrap.appendChild(el(`<article class="card clickable" data-search="${(p.name+' '+p.role+' '+p.type+' '+p.public).toLowerCase()}"><img class="portrait" src="${p.portrait}" alt="Retrato de ${p.name}">${tags([{text:p.type,cls:''}])}<h3>${p.name}</h3><p>${p.role}</p><a class="overlay" href="professor.html?id=${p.id}" aria-label="Abrir ${p.name}"></a></article>`))); searchable();
}
function renderProfessor(){
  ...
  app.innerHTML=`<p class="crumb"><a href="professores.html">Professores</a> / ${p.name}</p><section class="person"><img class="portrait" src="${p.portrait}" alt="Retrato de ${p.name}"><div><p class="eyebrow">${p.type}</p><h2>${p.name}</h2>${tags([{text:p.role,cls:''}])}<p class="lead">${p.public}</p><h3>O que Kiyomi manteria anotado</h3><p class="note">${p.kiyomi}</p><h3>Técnica conhecida ou inferida</h3><p>${p.technique}</p><h3>Observações de arquivo</h3><div class="list">${p.notes.map(n=>`<div class="list-item">${n}</div>`).join('')}</div></div></section>`;
}
